    ...
    const createQn_perSub = async (id, qn, options) => {
        try {
            ...
            if(json.success == true){
                ...
                setQns([...Qns, new_qn]);
                alert.info('New Question added !', {
                    timeout: 5000, // custom timeout just for this one alert
                    type: 'info',
                })
            }
            else{
                alert.error('Unable to create question !', {
                    timeout: 5000, // custom timeout just for this one alert
                    type: 'error',
                })
            }
    ...
    const createSubject = async (code, name) => {
        try {
            ...
            if(json.success ==true){
                ...
                alert.info('Subject created successfully!', {
                    timeout: 5000, // custom timeout just for this one alert
                    type: 'info',
                })  
            }
            else{
                alert.error('Invalid Subject details!', {
                    timeout: 5000, // custom timeout just for this one alert
                    type: 'error',
                })  
            }
    ...
    const deleteSubject = async (id) => {
        try {
            ...
            if(json.msg == "Success"){
                ...
                setSub(newSub);
                alert.success('Subject is removed!', {
                    timeout: 5000, // custom timeout just for this one alert
                    type: 'info',
                })  
            }
